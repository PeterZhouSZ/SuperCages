...
//   filename = "/media/jmt/DATA/Work/Models/warrok/warrok.obj";
   if (openFileSelectionDialog(filename, "Load Rig", "3D Meshes (*.obj *.ply)",(QWidget*)c->mainWindow))
   {
      ...
      //old loading
      //loadMesh(filename.c_str(),v,uv,f);
      ...
      loadSkeleton((filenameNoExt+"_skel.txt").c_str(),
                   joints,
                   fathers,
                   names);
      ...
      loadWeights((filenameNoExt+"_skelWeights.txt").c_str(),
                  c->skeletonWeights);
      ...
      //c->skeletonWeights->forcePartitionOfUnity();
      ...
      //Load CoR Weights
      std::ifstream fileCoR((filenameNoExt+"_corWeights.txt").c_str());

      if (fileCoR.is_open())
      {
         std::cout << "Load preexst. CoR Weights" << std::endl;
//         corWeights = new Weights(c->character->getNumVertices(),
//                                  c->character->getNumVertices());
//         loadWeights2((filenameNoExt+"_corWeights.txt").c_str(),
//                     corWeights);
//         if(c->isCharacterLoaded && c->isSkeletonLoaded)
//         {
//            CoRSkinning * cor = new CoRSkinning(skeletonWeights,
//                                                c->character,
//                                                c->skeleton,
//                                                corWeights);
//            c->cor = cor;
//         }
         ...
         loadSparseWeights((filenameNoExt+"_corWeights.txt").c_str(),
                            corWeightsWRTMesh);
         ...
         *(c->corWeights) = corWeightsWRTMesh * *(c->cageWeights);
         ...
         if(c->isCharacterLoaded && c->isSkeletonLoaded  &&  c->isCageLoaded)
         {
            ...
            std::cout << "!! Finished computing the cor weights matrix in " << nanoSec << " mSec." << std::endl;
         }
      ...
      std::ifstream fileMAXVOL((filenameNoExt+"_maxvol.txt").c_str());

      if (fileMAXVOL.is_open())
      {
          loadMAXVOL((filenameNoExt+"_maxvol.txt").c_str(),
